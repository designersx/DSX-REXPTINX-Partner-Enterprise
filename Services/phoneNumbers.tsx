...
export const fetchAvailablePhoneNumberByCountry = async (
  token,
  country_code,
  locality,
  administrative_area,
...
export const buyNumberFromEnterPrise = async (
  token,
  userId,
  phoneNumber,
  phone_number_type,
  ...
  try {
    const res = await axios.post(
      `${NEXT_PUBLIC_API_URL}/api/telnyx/buyNumberFromEnterPrise`,
...
export const assignNumberToAgent = async (token, inbound_agent_id, outbound_agent_id, phoneNumber, importStatus1) => {
  try {
    const res = await axios.post(
      `${NEXT_PUBLIC_API_URL}/api/telnyx/assignNumberFromEnterPrise`,
      {
...
export const numberRequirementsToBuyNumber = async (token, userId, country_code, phone_number_type, phoneNumber, data, import_type) => {
  try {
