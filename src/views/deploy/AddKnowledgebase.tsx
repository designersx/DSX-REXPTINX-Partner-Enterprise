...
export default function BasicModal({ open, onClose, onSubmit }) {
  ...
  const toggleOne = (url) => {
    setSelectedUrls((prev) => {
      ...
      localStorage.setItem('selectedSitemapUrls', JSON.stringify([...next].map((urlItem) => ({ url: urlItem, checkedStatus: true }))));
      return next;
    });
  };
    ...
    <Modal open={open} onClose={handleClose}>
      <MainCard title="Knowledge Base" modal darkTitle content={false} style={{ width: '30%' }}>
          ...
          {step === 2 && (
            <>
              ...
              <Box sx={{ mb: 2 }}>
                ...
                <input type="file" multiple accept=".png,.jpg,.jpeg,.pdf,.csv,.docx" onChange={handleFileChange} />
        ...
        <Stack direction="row" sx={{ gap: 1, justifyContent: 'flex-end', px: 2.5, py: 2 }}>
          ...
          {step === 1 ? (
            <Button variant="contained" size="small" onClick={() => kbName.trim() && setStep(2)} disabled={!kbName.trim()}>
              Next
            </Button>
          ) : (
            <Button
