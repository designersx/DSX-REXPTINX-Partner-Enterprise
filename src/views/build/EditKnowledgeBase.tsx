...
export default function EditKnowledgeBase({ open, onClose, onSubmit, knowledgeBase,setAlert }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      //   draggable: true,
      // });
      if(setAlert) setAlert("Knowledgebase Updated!")
      setTimeout(() => {
        handleClose();
      }, 2000);
    } catch (err) {
  ...
  return (
    ...
    >
      <MainCard title="Edit Knowledge Base" modal darkTitle content={false} style={{ width: '70%' }}>
        <CardContent>
          <Box sx={{ mb: 2 }}>
            ...
            <TextField
              ...
              size="small"
              value={currentKbName}
                  disabled={!!currentKbName} // ✅ disable if currentKbName has value
                    ...
                    <Box sx={{ maxHeight: 200, overflow: 'auto' }}>
                      <List>
                        ...
                        {sitemapUrls.map((sitemapUrl) => (
                          <ListItem key={sitemapUrl} disablePadding style={{ marginLeft: "10px" }}>
                            <ListItemIcon>
                              <Checkbox
                                edge="start"
        ...
        <Stack direction="row" sx={{ gap: 1, justifyContent: 'flex-end', px: 2.5, py: 2 }}>
          ...
          <Button
            ...
            disabled={
              !currentKbName.trim() ||
              getTotalSources() > MAX_SOURCES ||
              (isWebsiteValid && showSitemap && selectedUrls.size > 0 && newWebUrl) // 🚨 New condition
            }
          >
            Update
          </Button>
